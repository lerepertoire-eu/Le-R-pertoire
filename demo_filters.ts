...
const seed: { places: Place[] } = {
  places: [
    ...
    },
    {
      ...
      when_to_go:
        "Coucher de soleil d'avril à septembre ; dimanche pour ambiance plus calme.",
      access_booking:
    ...
    },
    {
      ...
      when_to_go:
        "Jeudi à samedi, résidence du DJ invité ; closing parties en septembre.",
      access_booking:
...
function formatPlace(place: Place): string {
  const firstTip = place.pro_tips[0] ?? "Aucun pro_tip";
  return `${place.destination} - ${place.name} - ${place.level_local} - ${firstTip}`;
}

function runQuery(label: string, filters: Parameters<typeof selectPlaces>[1], postFilter?: (places: Place[]) => Place[]) {
  const initialResults = selectPlaces(seed.places, filters);
  const results = postFilter ? postFilter(initialResults) : initialResults;

  console.log(`\n${label}`);
  console.log(`Résultats: ${results.length}`);
  results.forEach((place, index) => {
    console.log(`${index + 1}. ${formatPlace(place)}`);
  });
}

function main() {
  runQuery(
    '1) Paris romantique smart',
    { destination: 'Paris', formulas: ['romantique'] },
    (places) => places.filter((place) => place.level_local === 'smart'),
  );

  runQuery('2) Ibiza teuf_vip open late', {
    destination: 'Ibiza',
    formulas: ['teuf_vip'],
    openLate: true,
  });

  runQuery('3) Zermatt recherche "vue"', {
    destination: 'Zermatt',
    q: 'vue',
  });
}
